...
# Use in-memory index instead of file (since Render has ephemeral file system)
email_index = 0
...
def send_bulk_email(to_emails, subject, content):
    ...
    try:
        # Initialize Yagmail client directly
        ...
        # Move to next account for load balancing
...
def send_otp_email(to_email, otp):
    content = (
        ...
        f"Your OTP is: {otp}\n\n"
        f"Best regards,\nMedic Adrenaline.\n\n"
        f"📩 Tip: Click 'Report not spam' to always get your notifications fast!"
    )
...
def send_exam_pins_email(to_email, pins_dict):
    ...
    content += (
        "\n⚠️ Note: Do not disclose your PIN. Once it is activated on your device, "
        "it is tied to that device and cannot be reused elsewhere.\n\n"
        "If you need to move to a new device, please contact support.\n\n"
        "📩 Tip: Click 'Report not spam' to always get your notifications fast!\n\n"
...
def send_exam_pins_email_bulk(recipient_emails, pins_dict):
    ...
    content += (
        "\n⚠️ Note: Do not disclose your PIN. Once activated, it is tied to that specific device.\n\n"
        "📩 Tip: Click 'Report not spam' to always get your notifications fast!\n\n"
        "Best regards,\nMedic Adrenaline Team"
    )
